...
def _styles():
    ...
    custom = {
        ...
        "doc_title": ParagraphStyle("doc_title",
            fontSize=22, textColor=DARK_GRAY, fontName="Helvetica-Bold",
...
def _section_header(title: str, bg_color=DARK_GRAY):
    """Dark banner with white title text."""
...
def dispatch_table(dispatches: list[dict]):
    s = _styles()
...
def generate_report(call_data: dict, transcript: list[dict],
    ...
    story.append(HRFlowable(width="100%", thickness=1.5, color=DARK_GRAY))
    story.append(Spacer(1, 8))
    ...
    if dispatches:
        story.append(dispatch_table(dispatches))
    else:
    ...
    story.append(Paragraph(
        ...
        "This document was auto-generated by the AI-assisted Vigil system.",
        s["footer"]))
    doc.build(story)
